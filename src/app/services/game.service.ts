...
export class GameService {
  ...
  private started$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
  private cronoOn$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
  private readonly stopCrono$ = new Subject<void>();
  private readonly restartCrono$ = new Subject<void>();
  private tilesOk$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
  ...
  private initGameGrid() {
    ...
    this.tiles$ = Array(this.totalGameTiles$)
      ...
      .map(() => {
        const tileId = images
          .splice(Math.floor(Math.random() * images.length), 1)
          .toString();
  ...
  private getTimeString(): string {
    const minutes: string = `0${Math.floor(
      this.secondsPlayed$ / 60
    ).toString()}`.substr(-2);
    const seconds: string = `0${(this.secondsPlayed$ % 60).toString()}`.substr(
      -2
    );
